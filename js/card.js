        ...
        // ===========================================
        // TESTING GUIDE AND CONSOLE INSTRUCTIONS
        // ===========================================

        // Display testing instructions on page load
        console.log(`
════════════════════════════════════════════════════════════════
🚀 ADVANCED RATE LIMITING SYSTEM ACTIVATED
════════════════════════════════════════════════════════════════

✅ Features Implemented:
• Advanced device fingerprinting (canvas, WebGL, audio, etc.)
• Multi-layer storage (localStorage + sessionStorage + IndexedDB)
• Progressive penalties for bypass attempts
• Fuzzy fingerprint matching with confidence scoring
• Real-time countdown UI with Chinese messaging
• Tamper detection and validation

🧪 Testing Commands (open browser console):

Basic Testing:
• statusRateLimit()           - Show current rate limit status
• debugRateLimit()           - Enable debug mode for detailed logs
• clearRateLimit()           - Clear all rate limit data

Advanced Testing:
• CardRateLimit.admin.forceRateLimit(30)    - Force 30min cooldown
• CardRateLimit.admin.setCooldown(0.1)      - Set 6min cooldown for testing
• CardRateLimit.debug.fingerprint()         - Show fingerprint components
• CardRateLimit.admin.testFingerprint()     - Test fingerprint consistency

🔧 Configuration:
Current cooldown: ${RATE_LIMIT_CONFIG.cooldownHours} hour(s)
Debug mode: ${RATE_LIMIT_CONFIG.debugMode ? 'ON' : 'OFF'}

📱 Testing Scenarios:
1. Normal flow: Load page → see loading spinner → get card → reload → see rate limit
2. Different browsers: Test on Chrome vs Safari vs mobile browsers
3. Different devices: Test on mobile vs desktop
4. Storage persistence: Clear localStorage → check if limit persists

💡 Tips:
• Use debugRateLimit() to see detailed fingerprinting process
• Test in different browsers to verify fingerprint differences
• Try clearing individual storage types to test fallback mechanisms
• Loading spinner shows for 3 seconds minimum during card preparation
